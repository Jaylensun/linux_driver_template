...
static ssize_t dht11_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	mod_timer(&gpios[0].key_timer, jiffies + 10);//10 tick 100ms 推迟中断处理的时间，在定时器处理函数中解析数据
...
static void parse_dht11_datas(void)
{
	...
	// 解析数据 以高电平的时间长度判断数据data 1 依次移位获得最终数据
	/**此处2种情况
	 * 1. 中断次数满足84次，变会在中断处解析，此时删除dht11_read 函数启动的定时器
	 * 2. 中断次数不满足84次，10 tick定时时间到后，此时便会在定时器处理函数 key_timer_expire 解析数据，此时考虑总的中断次数为84次
	 *    遇到中断次数丢失但是中断次数足够保证40bit的数据完整，因此最少的中断次数是总中断次数减去起始信号的3次中断为84-3 = 81次
	 *    因此若中断次数少于81次不够解析完整数据，上面便会报错返回
	 **/
	for (i = g_dht11_irq_cnt - 80; i < g_dht11_irq_cnt; i+=2)
	{
		...
		if (high_time > 50000) /* data 1 高电平时间大于50us*/ 
		{
			data |= 1;
		}
	...
	if (crc == datas[4])
	{
		put_key(datas[0]); // 湿度整数部分
		put_key(datas[2]); // 温度整数部分
	}
	else
	{
		put_key(-1);
...
static irqreturn_t dht11_isr(int irq, void *dev_id)
{
	...
	{
		del_timer(&gpio_desc->key_timer); //中断次数正确，删除定时器（使用定时器防止中断次数丢失的情况）
		parse_dht11_datas();
	}
