...
static int gpio_drv_probe(struct platform_device *pdev)
{
    int err = 0;
    int i;
	struct device_node *np = pdev->dev.of_node; //设备树节点信息
	struct resource *res;
	...
	if (np)
	{
		...
		*/
		/*从设备树获得引脚信息*/
		count = of_gpio_count(np); //获得节点中 gpio 的序列号
		if (!count)
			return -EINVAL;

		gpios = kmalloc(count * sizeof(struct gpio_desc), GFP_KERNEL); //为用户定义gpio 描述结构体指针申请内存 
		for (i = 0; i < count; i++)
		{
			gpios[i].gpio = of_get_gpio(np, i);
			sprintf(gpios[i].name, "%s_pin_%d", np->name, i); //gpio引脚名字命名：由设备树节点的名字和设备编号组成
		}
...
static const struct of_device_id gpio_dt_ids[] = { //支持的设备
        { .compatible = "100ask,gpiodemo", },
        { /* sentinel */ }
};
